...
if ($manifest.product -ne 'OmsiLaunch' -or $manifest.product_version -ne '0.1.0-beta1' -or $manifest.package_alias -ne 'current') { throw 'Invalid release manifest identity.' }
...
$results = foreach ($file in Get-ChildItem -LiteralPath $extract -File -Recurse | Where-Object { $_.Extension -in '.exe', '.dll' -and $_.Name -ne 'nethost.dll' }) {
