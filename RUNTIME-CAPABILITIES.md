...
| Map read | `map.read` | `OmsiRuntimeReaders` | pinned `OmsiMap` layout | PASS; loaded state and tiles returned | RUNTIME_PASS |
...
| Scalar weather mutation | `weather.set` | `OmsiWeatherWriter` whitelist | upstream scalar fields and profile layouts | PASS: wind `0 -> 1 -> 0` | RUNTIME_PASS |
...
| D3D texture create/describe | typed opaque `D3DTextureHandle` | bounded native registry, dynamic default-pool texture | pinned texture semantics reconciled to Current | PASS: two formats, level metadata, multiple handles | RUNTIME_PASS |
| D3D texture update | typed `D3DTextureUpdate` | `GetLevelDesc`, bounds, `LockRect`, pitch-aware copy, `UnlockRect` | pinned DXHook behavior without its ownership defects | PASS: full and rectangular updates returned `S_OK` | RUNTIME_PASS |
| D3D texture release | typed release operation | atomic retirement and exactly-once COM release | owned-reference graph and generation model | PASS: release, repeated-release rejection and three reuse cycles | RUNTIME_PASS |
| D3D lifecycle events | public `RuntimeEvents` (`d3d.ready/lost/resetting/restored`) | Reset vtable observer plus ordered fixed transition queue | Current-build Reset slot ABI and D3D9 reset contract | `d3d.ready` PASS; loss/reset/restored not safely induced | PARTIALLY_SUPPORTED |
| D3D stale-handle rejection | session-tagged opaque handle plus device generation | native generation and resource state checks | fixed-width session channel and default-pool reset policy | PASS for released and prior-session handles; reset invalidation path not runtime exercised | PARTIALLY_SUPPORTED |
...
proven. Device loss, successful Reset, repeated loss/reset and request-vs-Reset
racing remain implemented but not runtime-proven because no safe, truthful
loss/reset producer was available; no synthetic Reset was invoked.
